package com.bbx.appstore.api.tools;
...
public class Device {
    ...
    public static String getCp(Context context) {
        ...
        String cp = getCpInSharePreference(context);
        if (cp != null)
            return cp;
        ...
        try {
            ...
            String ret = ai.metaData.getString("UMENG_CHANNEL");
            return null == ret ? "NA" : ret;
        } catch (Exception e) {
            return "NA";
        }
    }

    private static void setCpInSharePreference(Context context, String cp) {
        SharedPreferences sharedPreferences = context.getSharedPreferences(getPluginSharePreferenceName(), Context.MODE_PRIVATE);
        sharedPreferences.edit().putString("_p_cp", cp).apply();
    }

    private static String getCpInSharePreference(Context context) {
        SharedPreferences sharedPreferences = context.getSharedPreferences(getPluginSharePreferenceName(), Context.MODE_PRIVATE);
        return sharedPreferences.getString("_p_cp", null);
    }
