...
public class Report extends StoreApi {
    ...
    public void reportListUrl(Context context, String rtp_method, List<String> urlList, int replace, ClickInfo info) {
        Log.d(SConstant.TAG, "走到上流上报接口...");
        if (info != null && rtp_method != null) {

            Log.d(SConstant.TAG, "走到上流上报接口...并且点击坐标存在" + "x:" + info.x + "y:" + info.y + "replace:" + replace + "rtp_method" + rtp_method);
        }
        if (null != urlList) {
            if ("GET".equalsIgnoreCase(rtp_method)) {
                ...
                if (0 == replace) {
                    Log.d(SConstant.TAG, "replace为0...不替换宏不存在替换上报...");
                    noReplace(context, urlList);
                    return;
                }
                if (1 == replace) {
                    replace(context, urlList, info);
                    Log.d(SConstant.TAG, "替换宏并且上报Sucess...");
                }
            }
        }
    }
    ...
    private void replace(Context context, List<String> urlList, ClickInfo info) {
        ...
        for (String url : urlList) {
            if (null != info) {
                if (DEBUG) Log.d(SConstant.TAG, "替换url之前# POST url : " + url);
            ...
            if (DEBUG) Log.d(SConstant.TAG, "最终替换宏之后DB_STORE# POST url : " + url);
            requestForPost(context, url);
        }
    }
    ...
    private void requestForPost(Context context, String url) {
        ...
        if (DEBUG) Log.d(SConstant.TAG, "替换的参数后......");
        for (String param : params) {
            if (DEBUG) Log.d(SConstant.TAG, "params : " + param);
        ...
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                if (code == 200) {
                    if (DEBUG)
                        Log.d("Report", "DB_STORE# onSuccess onResponse : " + response.body());
                    Log.d(SConstant.TAG, "走到这里说明上报广告成功......");
                } else {
