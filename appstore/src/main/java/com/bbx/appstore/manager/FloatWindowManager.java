...
public class FloatWindowManager {
    ...
    public static int getWindowType(Context context) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if (Settings.canDrawOverlays(context) || context.checkSelfPermission(SYSTEM_ALERT_WINDOW) == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Log.d("ldl", "#FloatWindowManager#获取SYSTEM_ALERT_WINDOW权限了...");
                }
            }
        } else {
            type = WindowManager.LayoutParams.TYPE_SYSTEM_ERROR;
        ...
        if (TextUtils.equals(getDeviceBrand(), "SUGAR")) {
            type = WindowManager.LayoutParams.TYPE_SYSTEM_ERROR;
            Log.d("ldl", "#FloatWindowManager#定制化SUGAR设置type为TYPE_SYSTEM_ERROR");
        }
        Log.d("ldl", "#FloatWindowManager#获取当前的WindowType类型是..."+type);
        return type;
    }
    ...
    public void show(IFloatWindow window, Object extra) {
        ...
        if (!mViewMap.containsKey(window.getWindowId())) {
            if (DEBUG)
                Log.e(TAG, "#FloatWindowManager# show window : " + window.getWindowId());
