...
public class ApkUtils {
    ...
    public static String getVersionName(Context context, String pkgName) {
        ...
        if (isAvailable(context, pkgName)) {
            try {
                PackageInfo infoOld = pm.getPackageInfo(pkgName, 0);
                return infoOld.versionName;
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
            }
        }
...
//            Drawable icon = appInfo.loadIcon(pm);
    ...
    private interface InstallListener {

        void onAppInstalled(File file, String pkg);
    ...
    private static void directInstall(File file, Context context) {
        PackageInstallObserver pio = new PackageInstallObserver(file);
        PackageManager pm = context.getPackageManager();
        pm.installPackage(Uri.fromFile(file), pio, PackageManager.INSTALL_REPLACE_EXISTING, null);
    }
    ...
    public static List<LocalApp> scanThreeSystemAppList(Context context) {
